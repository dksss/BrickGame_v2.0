 ...
 *          (x, y) => (0, 1)
 */
static const int tetraminoes[FIGURES_COUNT][FIG_CEILS] = {
    {4, 6, 5, 7},  ///< I
    {0, 1, 5, 6},  ///< Z
    {2, 1, 5, 4},  ///< S
    {1, 5, 4, 6},  ///< T
...
void init_info(GameInfo_t *info) {
  info->field = get_array(S21_FIELD_HEIGHT, S21_FIELD_WIDTH);
...
void free_params(Game_params_t *params) {
  params->info->field = free_array(params->info->field, S21_FIELD_HEIGHT);
  params->info->next = free_array(params->info->next, FIGURE_HEIGHT);
}

void reset_tetris_game(Game_params_t *params) {
  params->state = START_GAME;

  clear_field(params->info->field, S21_FIELD_HEIGHT, S21_FIELD_WIDTH);
  params->info->score = 0;
  params->info->level = 1;
  params->info->speed = 1;
  params->info->pause = 0;
...
int get_hiscore() {
  ...
  FILE *hiscore_file = fopen(HISCORE_FILENAME, "r");
  if (!hiscore_file) {
...
void set_hiscore(int score) {
  FILE *hiscore_file = fopen(HISCORE_FILENAME, "w");
  fprintf(hiscore_file, "%d\n", score);
...
void spawn_figure(Game_params_t *params) {
  ...
  for (int i = 0; i < FIG_CEILS; i++) {
    params->figure->ceils[i].x = tetraminoes[params->figure->cur_fig][i] % 4 +
                                 (S21_FIELD_WIDTH - FIGURE_WIDTH) / 2;
...
bool check_gameover(GameInfo_t *info) {
  ...
  for (int i = 0; i < FIGURE_HEIGHT && !gameover; i++)
    for (int j = 0; j < S21_FIELD_WIDTH && !gameover; j++) {
...
bool check_collision(int **field, Figure_t *figure) {
  ...
  for (int i = 0; i < FIG_CEILS && check == false; i++) {
    if (figure->ceils[i].x < 0 || figure->ceils[i].x > S21_FIELD_WIDTH - 1)
      check = true;
    else if (figure->ceils[i].y < 0 ||
             figure->ceils[i].y > S21_FIELD_HEIGHT - 1)
      check = true;
  ...
  for (int i = 0; i < S21_FIELD_HEIGHT; i++) {
    ...
    while (filled) {
      ...
      for (int j = 0; j < S21_FIELD_WIDTH; j++) {
        ...
        if (filled_ceils == S21_FIELD_WIDTH) {
          filled_rows++;
...
void init_timer(struct timespec *begin) {
  if (begin->tv_sec == 0 && begin->tv_nsec == 0) {
    clock_gettime(CLOCK_MONOTONIC, begin);
  }
}

int is_shifting_time(struct timespec *start, const Game_params_t *params) {
  int result = 0;

  struct timespec current_time = {0};
  clock_gettime(CLOCK_MONOTONIC, &current_time);

  double timer = (current_time.tv_sec - start->tv_sec) +
                 (current_time.tv_nsec - start->tv_nsec) / 1.0E9;

  if (timer >= 1.0 - params->info->speed * 0.07) {
    result = 1;
    *start = current_time;
  }
